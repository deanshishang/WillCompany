...
class LoginReport:
	...
	def packWithHeader(self, info):
		...
		#???????????????????????
		ret = Tlv.pack(rpt_login_tag, osver+mac+ip+mask+gateway+ldns+appver)
		#print "Packwithheader: \n"
		#print ret
		#print "\n"
...
class DetectReport:
	...
	def unpack(self, datalen, data):
		...
		while(datalen - read_len - Tlv.tagsize - Tlv.lensize) > 0:
			tlv_tag, tlv_len, tlv_value = Tlv.unpack(data[read_len:])
			if tlv_tag == rpt_detectttime_tag:
				self.time = tlv_value
...
class Report:
	...
	def report(self, server, data):
		...
		try:
			...
			conn.connect()
			connected = True
			conn.request(method="GET", url=comp.path, headers=headers)
...
if __name__ == '__main__':
		rpt = Report()
		data = "1234567890"
		print rpt.report("http://download.rising.com.cn:80/do_not_delete/noc.gif",data);
